...
void aic_btsnoop_capture(unsigned char *p_buf, bool is_rcvd)
{

    switch(p_buf[0]){
        case HCI_COMMAND_PKT:
            aic_btsnoop_write_packet(HCI_COMMAND_PKT, (p_buf + 1), is_rcvd);
            break;
        case HCI_ACLDATA_PKT:
            aic_btsnoop_write_packet(HCI_ACLDATA_PKT, (p_buf + 1), is_rcvd);
            break;
        case HCI_SCODATA_PKT:
            aic_btsnoop_write_packet(HCI_SCODATA_PKT, (p_buf + 1), is_rcvd);
            break;
        case HCI_EVENT_PKT:
                aic_btsnoop_write_packet(HCI_EVENT_PKT, (p_buf + 1), false);
            break;
    }
