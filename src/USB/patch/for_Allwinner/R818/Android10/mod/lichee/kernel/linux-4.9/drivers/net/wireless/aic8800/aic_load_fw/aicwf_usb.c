...
static int aicwf_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if(testmode){
		if (rwnx_plat_bin_fw_upload_android(usb_dev, RAM_FW_ADDR, FW_RF_BASE_NAME_PC)) {
	        goto out_free_bus;
	    }

	    if(rwnx_plat_bin_fw_upload_android(usb_dev, FW_RAM_ADID_BASE_ADDR, FW_RF_ADID_BASE_NAME_PC)) {
	        goto out_free_bus;
	    }

	    if(rwnx_plat_bin_fw_upload_android(usb_dev, FW_RAM_PATCH_BASE_ADDR, FW_RF_PATCH_BASE_NAME_PC)) {
	        goto out_free_bus;
	    }
	}else{
	    ...
	    if(rwnx_plat_bin_fw_upload_android(usb_dev, FW_RAM_ADID_BASE_ADDR, FW_ADID_BASE_NAME_PC)) {
	        goto out_free_bus;
	    }

	    if(rwnx_plat_bin_fw_upload_android(usb_dev, FW_RAM_PATCH_BASE_ADDR, FW_PATCH_BASE_NAME_PC)) {
	        goto out_free_bus;
	    }
	}
#else
	...
	if(testmode){
		if(rwnx_plat_bin_fw_upload_android(usb_dev, FW_PATCH_TEST_BASE_ADDR, FW_PATCH_TEST_BASE_NAME_PC)) {
        	goto out_free_bus;
    	}
	}
