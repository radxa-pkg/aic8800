...
/*Produce ID struct of Bluetooth device */
typedef struct {
  bt_id_e id;     //id of bluetooth device
  char product_id[10];    //Product ID
}bt_device_s;

static int device_id = BT_INVALID_DEVICE;

extern int init_module(void *,unsigned long, const char *);
extern int delete_module(const char *,unsigned int);

/*Product ID of supported BT devices */
static bt_device_s bt_devices[] = {
  {BT_RTL8822BU,  "0bda:b82c"},
  {BT_RTL8822CU,  "0bda:c82c"},/*c82e*/
  {BT_RTL8822BEH, "10ec:b822"},
  {BT_RTL8723DU,  "0bda:d723"},
  {BT_MT7668U,  "0e8d:7668"},
  {BT_AIC8800D, "a69c:8801"}
};
//HISILICON add end
...
std::mutex init_mutex_;//add yangli
...
//HISILICON add begin
//BT.add BT devices detect
int bt_get_usb_device_id()
{
  int idnum;
  int i = 0;
  int ret = BT_INVALID_DEVICE;
  DIR *dir;
  struct dirent *next;
  FILE *fp = NULL;
  idnum = sizeof(bt_devices) / sizeof(bt_devices[0]);
  dir = opendir(USB_DIR);
  if (!dir){
    return BT_INVALID_DEVICE;
  }
  while ((next = readdir(dir)) != NULL){
    ...
    while (fgets(line,  sizeof(line), fp)){
      ...
      if (pos !=NULL){
        if (sscanf(pos + 8, "%x/%x/%x", &product_vid, &product_did, &product_bcddev)  <= 0)
          continue;
        sprintf(temp, "%04x:%04x", product_vid, product_did);
        for (i = 0; i < idnum; i++){
          if (strncmp(temp, bt_devices[i].product_id, 9) == 0){
            ret = bt_devices[i].id;
            break;
          }
        }
      }
      if (ret != BT_INVALID_DEVICE)
        break;
    }
    fclose(fp);
    if (ret != BT_INVALID_DEVICE)
...
int bt_get_pcie_device_id()
{
  ...
  while ((next = readdir(dir)) != NULL){
    char line[256];
    char uevent_file[256] = {0};
    sprintf(uevent_file, "%s/%s/uevent", PCIE_DIR, next->d_name);
    fp = fopen(uevent_file, "r");
    if (NULL == fp){
      continue;
    }
...
int bt_get_device_id()
{
  int ret = BT_INVALID_DEVICE;

  ret = bt_get_pcie_device_id();
  if (ret != BT_INVALID_DEVICE)
    return ret;

  ret = bt_get_usb_device_id();
  if (ret != BT_INVALID_DEVICE)
    return ret;
...
bool VendorInterface::Initialize(
    ...
    std::unique_lock<std::mutex> lock(init_mutex_);//add yangli
  if (g_vendor_interface) {
    ALOGE("%s: No previous Shutdown()?", __func__);
    return false;
  }
  device_id = bt_get_device_id();
  ALOGD("bluetooth device_id is %d", device_id);
  if (device_id == BT_INVALID_DEVICE) {
    ALOGE("%s: bluetooth device_id is NULL, check bluetooth devices!", __func__);
    if (property_set("persist.bluetooth.device", "false") < 0) {
      ALOGE("%s: Failed to set prop persist.bluetooth.device false", __func__);
    }
    return false;
  }

  if (property_set("persist.bluetooth.device", "true") < 0) {
    ALOGE("%s: set prop persist.bluetooth.device true is wrong!", __func__);
  }
