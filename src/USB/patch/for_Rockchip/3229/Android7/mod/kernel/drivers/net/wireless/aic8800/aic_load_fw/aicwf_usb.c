        ...
        */
        ...
        if (!wait_for_completion_interruptible(&bus->bustx_trgg)) {
			if (usbdev->app_cmp == false){
				usb_err("usb bustx thread will to stop\n");
				mdelay(100);
				usb_err("usb bustx thread stop\n");
				break;
				//continue;
...
static int patch_config(struct aic_usb_dev *usb_dev)
{
    ...
    //if (testmode) {
		...
		//if (testmode == FW_NORMAL_MODE) {
	        if((ret = rwnx_send_dbg_mem_write_req(usb_dev, 0x1e4d80, patch_addr))) {
	            printk("%x write fail\n", 0x1e4d80);
	        }
	        if((ret = rwnx_send_dbg_mem_write_req(usb_dev, 0x1e4d84, patch_num))) {
	            printk("%x write fail\n", 0x1e4d84);
	        }
