				      ...
				      char *ifname, size_t len)
{
	...
	if (os_strlen(ifname) >= IFNAMSIZ &&
		...
		res = os_snprintf(ifname, len, "p2p-%s-%d", ifname_ptr, wpa_s->p2p_group_idx);
...
static int wpas_p2p_create_iface(struct wpa_supplicant *wpa_s)
{
	...
	if (property_get("vendor.bcm_wifi", wifi_status, NULL) && strcmp(wifi_status, "bcm") != 0 && strcmp(wifi_status, "aic") != 0)
