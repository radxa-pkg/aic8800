...
#define WIFI_AIC_DRIVER_MODULE_PATH "/system/lib/modules/aic8800_fdrv.ko"
...
#ifdef WIFI_AIC_DRIVER_MODULE_PATH
static const char AIC_DRIVER_MODULE_PATH[]  = WIFI_AIC_DRIVER_MODULE_PATH;
#endif
...
int wifi_load_driver()
{
    ...
    if(wifi_type == AIC8800){
        if (insmod(AIC_DRIVER_MODULE_PATH, DRIVER_MODULE_ARG) < 0){
                ALOGE("%s aic8800_fdrv insmod fail \r\n", __func__);
                return -1;
        }
    }
...
int wifi_change_fw_path(const char *fwpath)
{
    ...
    wifi_type= check_wifi_chip_type();
    if(wifi_type == AIC8800){
	ALOGD("%s aic8800 not need to change fw \r\n", __func__);
    	return ret;
    }
